...
public class MaxHeap {
    ...
    // Agrega un nuevo valor al heap y reordena para mantener la propiedad máxima.
    ...
    public int poll() {
        ...
        int max = heap[0];          // Guardamos la raíz
    ...
    // Reordena hacia arriba: compara con el padre y sube mientras sea mayor.
    ...
    // Reordena hacia abajo: compara con los hijos y baja mientras sea menor.
    private void percolateDown(int indice) {
        while (true) {
            ...
            if (izquierdo < size && heap[izquierdo] > heap[mayor]) {
                mayor = izquierdo;
            }
            if (derecho < size && heap[derecho] > heap[mayor]) {
                mayor = derecho;
            }
    ...
    // Muestra el contenido actual del heap
    ...
    // Imprime el heap en forma de árbol (por niveles).
